...
class MandalaGenerator {
    ...
    drawMandala(pulse, rotation) {
        ...
        if (this.showHash && this.fullHash) {
            ...
            // Algorithm label
            this.ctx.fillText('MERIDIAN-HASH', this.canvas.width - 10, 20);
            
            // Full hash (wrapped if needed)
            const hashPart1 = this.fullHash.substring(0, 32);
            const hashPart2 = this.fullHash.substring(32);
            this.ctx.fillText(hashPart1, this.canvas.width - 10, 35);
            this.ctx.fillText(hashPart2, this.canvas.width - 10, 48);
            
            this.ctx.restore();
        }
        
        // Draw INTENTION TEXT in bottom left corner
        if (this.intentionText) {
            ...
            // Label
            this.ctx.fillText('INTENTION:', 10, this.canvas.height - 40);
            
            // Truncate intention if too long (max 80 chars for canvas display)
            let displayText = this.intentionText;
            if (displayText.length > 80) {
                displayText = displayText.substring(0, 77) + '...';
            }
            
            // Word wrap intention text (max 40 chars per line)
            const maxCharsPerLine = 40;
            const words = displayText.split(' ');
            let currentLine = '';
            let yPos = this.canvas.height - 25;
